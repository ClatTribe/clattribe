...
const AdsLandingPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/delhi_landing_page`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div ref={targetRef} className="min-h-screen bg-slate-950 text-white selection:bg-[#F59E0B] selection:text-slate-950 overflow-x-hidden">
      ...
      <section className="relative pt-24 pb-20 lg:pt-16 lg:pb-32 flex items-center">
          ...
          <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
            ...
            <motion.div style={{ opacity, scale }} className="lg:col-span-7 text-left">
              ...
              >
                ...
                <span className="block text-3xl md:text-4xl text-white/80 mt-4 font-bold tracking-tight">
                  Now in Delhi
                </span>
      ...
      {/* 5. FINAL CTA SECTION */}
      <section className="py-20 md:py-32 bg-[#F59E0B] text-slate-950 relative overflow-hidden">
  {/* Background decorative elements */}
  <div className="absolute top-10 left-10 text-[12rem] md:text-[20rem] font-black opacity-5 select-none pointer-events-none">"</div>
  ...
  <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
    <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
      ...
      <div className="text-left">
        <h2 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black mb-8 tracking-tighter uppercase leading-[0.9]">
          WINNING IS A <br/>
          <span className="underline decoration-[6px] decoration-slate-950 underline-offset-8">CHOICE.</span>
        </h2>
        
        <p className="text-xl md:text-2xl font-black mb-12 uppercase tracking-wider opacity-90">
        ...
        <button 
          ...
          className="group px-10 py-6 bg-slate-950 text-white rounded-[2rem] font-black text-xl md:text-2xl hover:scale-[1.02] transition-all shadow-2xl flex items-center gap-4 cursor-pointer mb-8 w-full sm:w-auto justify-center"
        >
          APPLY NOW 
          <ArrowRight className="w-7 h-7 group-hover:translate-x-1 transition-transform" />
        </button>

        {/* Bottom Alumni Badges */}
        <div className="flex flex-wrap gap-6 items-center mt-12 opacity-80">
          <div className="flex flex-col">
            <span className="text-4xl md:text-5xl font-black italic leading-none">AIR 1</span>
            <span className="text-xs font-bold uppercase tracking-widest mt-1">Pedigree</span>
          </div>
          <div className="flex flex-col">
            <span className="text-4xl md:text-5xl font-black italic leading-none">IIM-A</span>
            <span className="text-xs font-bold uppercase tracking-widest mt-1">Alumni</span>
          </div>
          <div className="flex flex-col">
            <span className="text-4xl md:text-5xl font-black italic leading-none">IIM L</span>
            <span className="text-xs font-bold uppercase tracking-widest mt-1">Standard</span>
          </div>
        </div>
      </div>

      {/* Right Content - Image Card + Call Button */}
      <div className="relative flex flex-col gap-6 items-center lg:items-end">
        
        {/* Mentor Card with Tilt Effect */}
        <motion.div 
          whileHover={{ rotate: -2, scale: 1.02 }}
          transition={{ type: "spring", stiffness: 300 }}
          className="relative bg-white rounded-[2.5rem] p-3 shadow-2xl max-w-sm w-full"
        >
          {/* Red notification dot */}
          <div className="absolute -top-3 -right-3 w-8 h-8 bg-red-500 rounded-full border-4 border-[#F59E0B] z-20"></div>
          
          {/* Square Image Container */}
          <div className="rounded-[2rem] overflow-hidden mb-4 aspect-square">
            <img 
              src="https://res.cloudinary.com/daetdadtt/image/upload/v1771051297/Gemini_Generated_Image_673glc673glc673g_beaji2.png" 
              alt="Deepak Sir - IIM Lucknow" 
              className="w-full h-full object-cover"
            />
          </div>
          ...
          <div className="text-center px-3 pb-2">
            <h3 className="text-2xl md:text-3xl font-black mb-1 text-slate-950">
              Deepak Sir
            </h3>
            ...
            <p className="text-sm text-slate-700 leading-relaxed italic">
              Master strategist with 10+ years crafting AIR 1 blueprints. Transforms analytical potential into championship performance.
            </p>
          </div>
        </motion.div>
        ...
        >
          ...
          {/* Phone Icon Circle */}
          <div className="w-14 h-14 md:w-16 md:h-16 bg-[#F59E0B] rounded-full flex items-center justify-center flex-shrink-0 relative z-10">
            <Phone className="w-7 h-7 md:w-8 md:h-8 text-slate-950" fill="currentColor" />
          </div>

          {/* Text content */}
          <div className="flex flex-col items-start relative z-10 text-left">
            <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover:text-[#F59E0B] transition-colors">
              Call Now
            </span>
          ...
          {/* Arrow Icon */}
          <ArrowRight className="ml-auto w-5 h-5 md:w-6 md:h-6 text-slate-700 group-hover:text-white transition-colors" />
        </motion.a>
      </div>
    </div>
  </div>
