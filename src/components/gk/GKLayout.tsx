...
export default function GKLayout({
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (editResp.ok) {
          ...
          editRows.forEach((r) =>
            results.push({ id: r.id, title: r.title, type: "editorial", tab: "editorial" })
          );
        }
        ...
        if (flashResp.ok) {
          ...
          flashRows.forEach((r) =>
            results.push({ id: r.id, title: r.front, type: "flashcard", tab: "flashcards" })
          );
        }
  ...
  const handleSignOut = async () => {
    ...
    const _ls =
      typeof window !== "undefined"
        ? window.localStorage
        : { getItem: () => null, setItem: () => null, removeItem: () => null };
    _ls.removeItem("gk_userName");
    _ls.removeItem("gk_userEmail");
    _ls.removeItem("gk_isSubscribed");
    _ls.removeItem("gk_registrationDate");
  ...
  const navLinks = [
    ...
    { id: "monthly-summary", name: "Monthly Summary", icon: CalendarDays, badge: null },
    { id: "static-gk", name: "Static GK", icon: Scale, badge: "NEW" },
  ];

  return (
    <div
      className={`flex flex-col gk-portal${
        theme === "dark" ? " dark" : ""
      } min-h-screen bg-white dark:bg-[#060818] font-sans text-[#060818] dark:text-white transition-colors duration-300 relative`}
    >
      {toast && (
        <motion.div
      ...
      <header className="sticky top-0 z-[60] bg-[#060818]/80 backdrop-blur-md border-b border-white/[0.08] shadow-[0_1px_24px_rgba(0,0,0,0.45)]">
        <div className="mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-5 text-[13px] text-white/75">
            <a href="https://www.clattribe.com" className="hover:text-white transition-colors">
              Home
            </a>
      ...
      <div className="flex flex-1 relative">
        ...
        <nav
          className={`fixed md:sticky md:top-[60px] inset-y-0 left-0 z-[80] bg-white dark:bg-[#060818] border-r border-gray-100 dark:border-white/5 h-full md:h-[calc(100vh-60px)] flex flex-col overflow-y-auto no-scrollbar w-64 md:w-56 shrink-0 transition-all duration-300 transform ${
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-sm font-medium leading-none flex-1">{link.name}</span>
                  {link.badge && !isActive && (
