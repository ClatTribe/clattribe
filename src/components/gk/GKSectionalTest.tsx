...
import {
  ArrowLeft, ArrowRight, CheckCircle2, XCircle, Clock, Trophy,
  Zap, BookOpen, ChevronRight, Timer, AlertCircle, Lightbulb,
  RefreshCw, Target
} from 'lucide-react';
...
const SET_LABELS = [
  ...
  { num: 3, label: 'Set 3', theme: 'Environment & Economy', color: 'bg-green-500' },
  { num: 4, label: 'Set 4', theme: 'Critical Analysis', color: 'bg-[#F59E0B]' },
  { num: 5, label: 'Set 5', theme: 'Synthesis & Strategy', color: 'bg-[#060818]' },
];
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-black text-[#060818] dark:text-white">GK Sectional Tests</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {SET_LABELS.map((set, idx) => (
          ...
          >
            <div className={`absolute -right-6 -top-6 w-24 h-24 ${set.color} opacity-5 group-hover:opacity-10 rounded-full transition-all`} />
            <div className="space-y-4 relative">
              <div className={`w-12 h-12 ${set.color} rounded-2xl flex items-center justify-center text-white font-black text-lg shadow-lg`}>
                {set.num}
              </div>
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 font-medium mt-1">{set.theme}</p>
              </div>
              <div className="flex items-center justify-between pt-3 border-t border-gray-50 dark:border-white/5">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">30 Questions</span>
                <ChevronRight size={16} className="text-gray-300 group-hover:text-[#F59E0B] group-hover:translate-x-1 transition-all" />
              </div>
            </div>
          </motion.button>
        ))}
      ...
      <div className="bg-amber-50 dark:bg-[#F59E0B]/10 border border-amber-100 dark:border-[#F59E0B]/20 rounded-2xl p-4 flex items-start gap-3">
        ...
        <p className="text-xs text-amber-800 dark:text-[#F59E0B]/80 font-medium">
          Each set covers 30 questions across Polity, Economy, International Affairs, Environment, Science & Technology, and Sports — all from Jan–Apr 2026.
        </p>
      </div>
    </motion.div>
  );
...
function QuizScreen({
  ...
  const formatTime = (s: number) => `${Math.floor(s / 60).toString().padStart(2, '0')}:$x(s % 60).toString().padStart(2, '0')}`;
      ...
      return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          <div className="flex items-center justify-between">
            <button
              onClick={() => { setReviewMode(false); setCurrent(0); }}
              className="flex items-center gap-2 text-gray-500 hover:text-[#F59E0B] transition-colors font-bold uppercase text-[10px] tracking-widest"
            >
          ...
          <div className="bg-white dark:bg-white/5 rounded-3xl border border-gray-100 dark:border-white/10 p-6 space-y-5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 bg-gray-100 dark:bg-white/10 px-2 py-1 rounded-lg">{q.topic}</span>
            </div>
            <p className="font-bold text-[#060818] dark:text-white text-base leading-relaxed">{q.question}</p>
            <div className="space-y-3">
              {q.options.map((opt, i) => (
                <div
                  key={i}
                  className={`p-4 rounded-2xl border-2 transition-all font-medium text-sm ${
                    i === q.correct
                      ? 'bg-green-50 dark:bg-green-500/10 border-green-500 text-green-800 dark:text-green-300'
                      : i === userAns && !correct
                      ? 'bg-red-50 dark:bg-red-500/10 border-red-400 text-red-800 dark:text-red-300'
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span>{opt}</span>
                    {i === q.correct && <CheckCircle2 size={16} className="ml-auto text-green-500 flex-shrink-0" />}
                    {i === userAns && !correct && i !== q.correct && <XCircle size={16} className="ml-auto text-red-500 flex-shrink-0" />}
                  </div>
                </div>
              ))}
            </div>
            <div className="bg-amber-50 dark:bg-[#F59E0B]/10 border border-amber-100 dark:border-[#F59E0B]/20 rounded-2xl p-4">
              <p className="text-xs font-black text-[#F59E0B] uppercase tracking-widest mb-2">Explanation</p>
              <p className="text-sm text-amber-900 dark:text-[#F59E0B]/80 font-medium leading-relaxed">{q.explanation}</p>
            </div>
          </div>
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
        <div className="bg-[#060818] p-8 rounded-3xl text-center space-y-6">
          ...
          <div>
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">GK Sectional — Set {setNum}</p>
            <p className="text-5xl font-black text-white mt-2">{pct}%</p>
            <p className="text-gray-400 font-bold mt-1">{score} / {questions.length} correct</p>
          </div>
          <div className="flex justify-center gap-6 border-t border-white/10 pt-6">
            <div className="text-center">
              <p className="text-[#F59E0B] font-black text-lg">{formatTime(timeSpent)}</p>
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold">Time</p>
            </div>
            <div className="text-center">
              <p className="text-[#F59E0B] font-black text-lg">{Math.round(timeSpent / questions.length)}s</p>
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold">Per Q</p>
            </div>
            <div className="text-center">
              <p className="text-[#F59E0B] font-black text-lg">{questions.length - score}</p>
              <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold">Wrong</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 bg-gray-100 dark:bg-white/10 px-3 py-1.5 rounded-xl">
            ...
            <span className="font-black text-[#060818] dark:text-white text-sm">{formatTime(timeSpent)}</span>
          </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-lg ${
              q.difficulty === 'easy' ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' :
              q.difficulty === 'hard' ? 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400' :
              'bg-amber-100 text-amber-700 dark:bg-[#F59E0B]/20 dark:text-[#F59E0B]'
            }`}>{q.difficulty}</span>
            <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 bg-gray-100 dark:bg-white/10 px-2 py-1 rounded-lg">{q.topic}</span>
          </div>

          <p className="text-base font-bold text-[#060818] dark:text-white leading-relaxed">{q.question}</p>

          <div className="space-y-3">
            {q.options.map((opt, i) => {
              let cls = 'bg-gray-50 dark:bg-white/5 border-transparent text-gray-700 dark:text-gray-300 hover:border-[#F59E0B]/50 hover:bg-[#F59E0B]/5';
              if (answered) {
                if (i === q.correct) cls = 'bg-green-50 dark:bg-green-500/10 border-green-500 text-green-800 dark:text-green-300';
                else if (i === selectedAnswer) cls = 'bg-red-50 dark:bg-red-500/10 border-red-400 text-red-700 dark:text-red-300';
                else cls = 'bg-gray-50 dark:bg-white/5 border-transparent text-gray-400 dark:text-gray-500 opacity-60';
              } else if (selectedAnswer === i) {
                cls = 'bg-[#F59E0B]/10 border-[#F59E0B] text-[#060818] dark:text-white';
              }
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {answered && i === q.correct && <CheckCircle2 size={16} className="text-green-500 flex-shrink-0" />}
                    {answered && i === selectedAnswer && !isCorrect && <XCircle size={16} className="text-red-500 flex-shrink-0" />}
                  </div>
                </motion.button>
              );
            })}
            ...
            {showExplanation && (
              ...
              >
                <div className={`rounded-2xl p-4 border ${
                  ...
                  <p className="text-xs font-medium leading-relaxed text-gray-700 dark:text-gray-300">{q.explanation}</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
...
export default function GKSectionalTest({ onComplete, onBack }: GKSectionalTestProps) {
  ...
  const loadSet = async (setNum: number) => {
    ...
    try {
      ...
      if (err) throw err;
      if (!data || data.length === 0) throw new Error('No questions found for this set.');
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-sm font-bold text-gray-500 dark:text-gray-400">Loading questions…</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
        ...
        <button
          onClick={() => { setError(null); setSelectedSet(null); }}
