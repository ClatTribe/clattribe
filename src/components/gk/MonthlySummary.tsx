...
interface NewsItem {
  ...
  content: string;
  image?: string;
  clatProbability: 'High' | 'Medium' | 'Low';
}

function CLATProbabilityBadge({ level }: { level: 'High' | 'Medium' | 'Low' }) {
  const styles = {
    High: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20',
    Medium: 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20',
    Low: 'bg-gray-100 dark:bg-white/5 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-white/10',
  };
  const dots = {
    High: 'bg-emerald-500',
    Medium: 'bg-amber-500',
  ...
  return (
    <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-[10px] font-black uppercase tracking-widest ${styles[level]}`}>
...
const MONTHLY_NEWS: NewsItem[] = [
  {
    ...
    content: `
      ...
      Legal Implications for CLAT:
      Students should focus on Article 200 and Article 201 of the Constitution. Understand the difference between "withholding assent," "granting assent," and "reserving for the President." This judgment is a direct application of the principle of "Constitutional Morality" and "Federalism."
    `,
    image: 'https://picsum.photos/seed/court/800/400',
    clatProbability: 'High',
  },
  {
    id: '2',
    ...
    content: `
      ...
      Privacy is a fundamental right under Article 21 (Puttaswamy judgment). The DPDP Act is the legislative realization of this right. Expect questions on the "Data Protection Board," "Consent Architecture," and "Cross-border data transfers."
    `,
  ...
  },
  {
    ...
    content: `
      ...
      CLAT Focus:
      Understand the difference between a Free Trade Agreement (FTA) and a TEPA. Note the member countries of EFTA (often confused with the EU). Focus on the "Investment Chapter" which is a unique feature of this agreement.
    `,
    image: 'https://picsum.photos/seed/trade/800/400',
    clatProbability: 'Medium',
  },
];
      ...
      >
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <CLATProbabilityBadge level={selectedNews.clatProbability} />
          </div>

          <h1 className="text-4xl font-black text-[#060818] dark:text-white leading-tight">
            {selectedNews.title}
          </h1>

          {selectedNews.image && (
            <div className="aspect-video rounded-[2.5rem] overflow-hidden border border-gray-100 dark:border-white/10 shadow-2xl">
              <img
    ...
    <div className="space-y-12">
        ...
        <div className="flex bg-gray-100 dark:bg-white/5 p-1.5 rounded-2xl border border-gray-200 dark:border-white/10 self-start">
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${
              view === 'news'
                ? 'bg-white dark:bg-[#F59E0B] text-[#060818] shadow-sm'
                : 'text-gray-500 hover:text-[#060818] dark:hover:text-white'
            }`}
          >
            <BookOpen size={18} /> News Updates
          </button>
          <button
      ...
      <AnimatePresence mode="wait">
        {view === 'news' ? (
          ...
          >
            {MONTHLY_NEWS.map((news) => (
              <motion.div
                ...
                className="group bg-white dark:bg-white/5 rounded-[2.5rem] border border-gray-100 dark:border-white/10 overflow-hidden cursor-pointer hover:shadow-2xl hover:shadow-[#F59E0B]/10 transition-all duration-300"
              >
                ...
                <div className="p-8 space-y-4">
                  ...
                  <h3 className="text-xl font-black text-[#060818] dark:text-white group-hover:text-[#F59E0B] transition-colors leading-tight line-clamp-2">
                    {news.title}
                  </h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 font-medium">
                    {news.summary}
                  </p>
                  <CLATProbabilityBadge level={news.clatProbability} />
                  <div className="pt-2 flex items-center gap-2 text-[#F59E0B] font-bold text-xs uppercase tracking-widest">
          ...
          >
            {!showResult ? (
              <div className="bg-white dark:bg-white/5 rounded-[3rem] border border-gray-100 dark:border-white/10 p-12 space-y-10 shadow-2xl">
                {/* Quiz Header */}
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <div className="h-1.5 w-48 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden">
                      <motion.div
                  ...
                  <div className="flex items-center gap-2 bg-[#F59E0B]/10 px-4 py-2 rounded-2xl">
                    <Trophy size={18} className="text-[#F59E0B]" />
                    <span className="text-sm font-black text-[#F59E0B]">{score} Points</span>
                  </div>
                </div>

                {/* Question */}
                <h2 className="text-2xl font-black text-[#060818] dark:text-white leading-tight">
                  ...
                  {MONTHLY_QUIZ[currentQuestion].options.map((option, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full p-6 rounded-[1.5rem] border-2 text-left transition-all flex items-center justify-between group ${bgColor} ${borderColor} ${textColor}`}
                      >
                        <span className="font-bold text-lg">{option}</span>
                        {isAnswered && isCorrect && <CheckCircle2 size={24} className="text-green-500" />}
                        {isAnswered && isSelected && !isCorrect && <AlertCircle size={24} className="text-red-500" />}
                      </button>
                    );
                  })}
                </div>
                ...
                {isAnswered && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="p-6 rounded-2xl bg-[#060818]/5 dark:bg-white/5 border-l-4 border-[#F59E0B] space-y-2"
                  >
                ...
                {isAnswered && (
                  <motion.button
                    ...
                    className="w-full bg-[#060818] dark:bg-[#F59E0B] text-white dark:text-[#060818] py-5 rounded-2xl font-black text-lg shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3"
                  >
              ...
              <div className="bg-white dark:bg-white/5 rounded-[3rem] border border-gray-100 dark:border-white/10 p-16 text-center space-y-8 shadow-2xl">
                ...
                <div className="space-y-2">
                  <h2 className="text-4xl font-black text-[#060818] dark:text-white">Quiz Completed!</h2>
                  <p className="text-gray-500 dark:text-gray-400 font-medium text-lg">Great effort on staying updated with April's news.</p>
                </div>

                <div className="text-6xl font-black text-[#F59E0B]">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center pt-8">
                  <button
                    onClick={resetQuiz}
                    className="px-10 py-4 bg-[#060818] dark:bg-white/10 text-white rounded-2xl font-bold hover:bg-gray-800 dark:hover:bg-white/20 transition-all"
                  >
                    Try Again
                  </button>
                  <button
