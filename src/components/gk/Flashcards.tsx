...
function saveSM2Cards(cards: SM2Card[]) {
  (typeof window !== "undefined"
    ? window.localStorage
    : { getItem: () => null, setItem: () => null, removeItem: () => null }
  ).setItem("ct_sm2", JSON.stringify(cards));
}
...
function getDailyCards(all: FlashCard[]): FlashCard[] {
  ...
  const rest = all
    .filter((c) => !todayDue.includes(c.id))
    .sort(() => rng() - 0.5);
...
function SwipeCard({
  ...
  return (
    <motion.div
      ...
      style={{
        x: isDraggable ? x : undefined,
        rotate: isDraggable ? rotate : undefined,
        zIndex: 10,
      }}
    ...
    >
      ...
      >
        {/* GOT IT stamp */}
        <motion.div
          className="absolute top-8 left-6 px-3 py-1 rounded-lg border-4 z-20"
          style={{
            opacity: gotItOp,
            rotate: -22,
            borderColor: "#16a34a",
        ...
        {/* REVIEW stamp */}
        <motion.div
          className="absolute top-8 right-6 px-3 py-1 rounded-lg border-4 z-20"
          style={{
            opacity: revOp,
            rotate: 22,
            borderColor: "#dc2626",
          ...
          {!isFlipped ? (
            ...
            >
              <div className="flex-1 flex flex-col items-center justify-center p-7 gap-4">
                ...
                <span style={{ color: "#94a3b8", fontSize: 12, marginTop: 8 }}>
                  Tap to reveal · Swipe to judge
                </span>
              </div>
              <div
                style={{
...
function FlipCard({
  ...
  return (
    <div
      className="relative w-full max-w-[320px] mx-auto h-[340px] cursor-pointer select-none"
      onClick={onFlip}
...
export default function Flashcards() {
  ...
  const toggleFlashBookmark = (id: number) => {
    setBookmarkedIds((prev) => {
      const next = prev.includes(String(id))
        ? prev.filter((x: string) => x !== String(id))
        : [...prev, String(id)];
      (typeof window !== "undefined"
        ? window.localStorage
        : { getItem: () => null, setItem: () => null, removeItem: () => null }
      ).setItem("gk_flash_bookmarks", JSON.stringify(next));
      return next;
    });
  };
  const isSubscribed =
    ...
    handleSwipeRef.current = (dir: "left" | "right") => {
      ...
      if (dir === "left") {
        ...
        setMastered((prev) => {
          ...
          ).setItem("ct_mastered_v2", JSON.stringify(Array.from(next)));
          return next;
        });
      }
  ...
  const handleDailyAnswer = (knew: boolean) => {
    ...
    const sm2 = getSM2Cards();
    const existing = sm2.find((c) => c.id === card.id) || {
      id: card.id,
      ef: 2.5,
      interval: 0,
      due: "",
    ...
    if (dailyIdx + 1 >= dailyCards.length) {
      ...
      (typeof window !== "undefined"
        ? window.localStorage
        : { getItem: () => null, setItem: () => null, removeItem: () => null }
      ).setItem("ct_streak", String(newStreak));
      (typeof window !== "undefined"
        ? window.localStorage
        : { getItem: () => null, setItem: () => null, removeItem: () => null }
      ).setItem("ct_last_daily", today);
      setDailyDone(true);
    } else {
      setDailyIdx((i) => i + 1);
      setDailyFlipped(false);
    ...
    if (knew) {
      ...
      if (next > best) {
        (typeof window !== "undefined"
          ? window.localStorage
          : { getItem: () => null, setItem: () => null, removeItem: () => null }
        ).setItem("ct_speed_best", String(next));
        setSpeedBest(next);
      }
    }
    if (speedIdx + 1 >= speedCards.length) {
      endSpeed();
      return;
    }
    setSpeedIdx((i) => i + 1);
  ...
  const overallPct = allCards.length
    ? Math.round((totalMastered / allCards.length) * 100)
    : 0;
  ...
  if (mode === "daily") {
    ...
    return (
      <div className="flex flex-col items-center gap-5 py-4 px-4 max-w-lg mx-auto">
        ...
        <div className="w-full">
          ...
          >
            <div
              style={{
                height: "100%",
                width: `${(dailyIdx / dailyCards.length) * 100}%`,
                background: "linear-gradient(to right,#fbbf24,#f59e0b)",
                borderRadius: 999,
        ...
        />
        <button
          onClick={() => toggleFlashBookmark(card.id)}
          title="Bookmark"
          className={`p-2 rounded-xl border transition-all ${bookmarkedIds.includes(String(card.id)) ? "bg-amber-50 dark:bg-amber-500/10 border-amber-300 dark:border-amber-500/30 text-amber-500" : "bg-gray-50 dark:bg-white/5 border-gray-100 dark:border-white/10 text-gray-400 hover:text-amber-500"}`}
        >
          <BookMarked size={16} />
        </button>
  ...
  if (mode === "speed") {
    ...
    const timerColor =
      speedTimer > 30 ? "#4ade80" : speedTimer > 10 ? "#fbbf24" : "#ef4444";

    if (speedDone) {
      const attempted = speedIdx + 1;
      const accuracy =
        attempted > 0 ? Math.round((speedCorrect / attempted) * 100) : 0;
      ...
      return (
        <div className="flex flex-col items-center gap-6 py-6 px-4 max-w-lg mx-auto">
          ...
          >
            ...
            <p style={{ color: "#f8fafc", fontSize: 22, fontWeight: 800 }}>
              Time&apos;s Up!
            </p>
            <p
              style={{
            ...
            <div className="flex justify-center gap-8 mt-5">
              <div className="text-center">
                <p style={{ color: "#60a5fa", fontSize: 22, fontWeight: 800 }}>
                  {accuracy}%
                </p>
                <p style={{ color: "#94a3b8", fontSize: 12 }}>Accuracy</p>
              </div>
              <div className="text-center">
                <p style={{ color: "#a78bfa", fontSize: 22, fontWeight: 800 }}>
                  {attempted}
                </p>
                <p style={{ color: "#94a3b8", fontSize: 12 }}>Attempted</p>
              </div>
              <div className="text-center">
                <p style={{ color: "#fbbf24", fontSize: 22, fontWeight: 800 }}>
                  {speedBest}
                </p>
          ...
          <p style={{ color: "#64748b", fontSize: 13, textAlign: "center" }}>
            {accuracy >= 80
              ? "🔥 Excellent! You're CLAT-ready!"
              : accuracy >= 60
                ? "💪 Good job! Keep pushing!"
                : "📚 Accuracy over speed. Practise more."}
          </p>
      ...
      return (
        <div className="flex flex-col items-center gap-6 py-6 px-4 max-w-lg mx-auto">
          ...
          >
            ...
            <div className="flex justify-center gap-8 mt-5">
              <div className="text-center">
                <p style={{ color: "#4ade80", fontSize: 26, fontWeight: 800 }}>
                  {swipeCards.length - missedIds.length}
                </p>
                <p style={{ color: "#94a3b8", fontSize: 12 }}>Knew it ✓</p>
              </div>
              <div className="text-center">
                <p style={{ color: "#f87171", fontSize: 26, fontWeight: 800 }}>
                  {missedIds.length}
                </p>
          ...
          <div className="flex gap-3 w-full">
            {missedCards.length > 0 && (
              <button
                onClick={() => startBrowse(missedCards)}
                style={{
                  flex: 1,
                  padding: "12px 0",
                  borderRadius: 12,
          ...
          >
            ...
            {swipeIdx + 1 < swipeCards.length && (
              <div
                className="absolute inset-0 rounded-3xl"
                style={{
                  transform: "scale(0.93) translateY(11px)",
                  zIndex: 2,
                  transformOrigin: "bottom center",
                  backgroundColor: "#f1f5f9",
    ...
    return (
      <div className="flex flex-col gap-5 pt-4 pb-8 px-4 max-w-lg mx-auto">
        ...
        >
          ...
          >
            {totalMastered} of {allCards.length} cards mastered
          </p>
        </div>
  ...
  // HOME
  // ══════════════════════════════════════════════════════════════════════════
  const todayDailyDone =
    typeof window !== "undefined" &&
    (typeof window !== "undefined"
      ? window.localStorage
      : { getItem: () => null, setItem: () => null, removeItem: () => null }
    ).getItem("ct_last_daily") === new Date().toISOString().split("T")[0];
  ...
  return (
    <div className="flex flex-col gap-5 py-4 px-4 max-w-lg mx-auto">
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p style={{ color: "#0284c7", fontSize: 11, margin: 0 }}>
        ...
        >
          🃏 Browse all {allCards.length} cards · Tinder-style · Missed cards
          auto-saved
        </p>
      </button>
      ...
      >
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p style={{ color: "#0d9488", fontSize: 11, margin: 0 }}>
              {totalMastered} / {allCards.length} cards mastered
            </p>
          </div>
        </div>
        <div
      ...
      >
        ...
        >
          CLAT GK tests current affairs from the last 12 months. Do your Daily
          Dose every morning — 15 minutes a day for 90 days covers the entire
          syllabus with spaced repetition.
        </p>
      </div>
    </div>
  );
