...
const WEEK_LABELS = [
  { week: 1, label: "Week 1", desc: "Jan 2026 Wk 1" },
  { week: 2, label: "Week 2", desc: "Jan 2026 Wk 2" },
  { week: 3, label: "Week 3", desc: "Jan 2026 Wk 3" },
  { week: 4, label: "Week 4", desc: "Jan 2026 Wk 4" },
  { week: 5, label: "Week 5", desc: "Feb 2026 Wk 1" },
  { week: 6, label: "Week 6", desc: "Feb 2026 Wk 2" },
  { week: 7, label: "Week 7", desc: "Feb 2026 Wk 3" },
  { week: 8, label: "Week 8", desc: "Feb 2026 Wk 4" },
  { week: 9, label: "Week 9", desc: "Mar 2026 Wk 1" },
  { week: 10, label: "Week 10", desc: "Mar 2026 Wk 2" },
  { week: 11, label: "Week 11", desc: "Mar 2026 Wk 3" },
  { week: 12, label: "Week 12", desc: "Mar 2026 Wk 4" },
];
      ...
      <div className="flex items-center justify-between">
        ...
        <div className={onBack ? "" : "w-full"}>
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium text-sm mt-1">
            {" "}
            Fresh current affairs questions — one quiz per week.{" "}
          </p>{" "}
        </div>{" "}
      </div>{" "}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        {WEEK_LABELS.map((w, idx) => (
          <motion.button
            ...
            className="group bg-white dark:bg-white/5 p-6 rounded-3xl border border-gray-100 dark:border-white/10 text-left hover:shadow-xl hover:shadow-[#F59E0B]/10 hover:border-[#F59E0B]/30 transition-all duration-300 space-y-4"
          >
            {" "}
            <div className="w-12 h-12 bg-[#F59E0B]/10 rounded-2xl flex items-center justify-center group-hover:bg-[#F59E0B] transition-colors">
              {" "}
              <Calendar
                size={22}
                className="text-[#F59E0B] group-hover:text-[#060818] transition-colors"
              />{" "}
            </div>{" "}
            <div>
              {" "}
...
function QuizScreen({
  ...
  const handleFinish = (finalAnswers: (number | null)[]) => {
    ...
    }, 0); /* Write analytics to localStorage */
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        >
          {" "}
          <ArrowLeft size={14} /> Week {weekNum}{" "}
        </button>{" "}
        <div
          className={`flex items-center gap-2 px-4 py-2 rounded-2xl font-black text-sm ${timeLeft < 60 ? "bg-red-500/10 text-red-500" : "bg-[#F59E0B]/10 text-[#F59E0B]"}`}
        >
          {" "}
          <Timer size={16} /> {mins}:{secs.toString().padStart(2, "0")}{" "}
        </div>{" "}
      </div>{" "}
      {/* Progress Bar */}{" "}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {" "}
          {q.topic && (
            <span className="inline-block text-[10px] font-black uppercase tracking-widest text-[#F59E0B] bg-[#F59E0B]/10 px-3 py-1 rounded-full">
              {" "}
              {q.topic}{" "}
            </span>
          )}{" "}
          ...
          <div className="space-y-3">
            ...
            {q.options.map((opt, i) => {
              let style =
                "bg-gray-50 dark:bg-white/5 border-gray-100 dark:border-white/10 text-gray-700 dark:text-gray-300 hover:border-[#F59E0B]/40";
              if (answered) {
                if (i === q.correct)
                  style =
                  ...
                  style =
                    "bg-gray-50 dark:bg-white/5 border-gray-100 dark:border-white/10 text-gray-400 opacity-60";
              }
              return (
                <motion.button
                  key={i}
                ...
                >
                  {" "}
                  <span
                    className={`w-7 h-7 rounded-xl flex items-center justify-center text-[11px] font-black shrink-0 ${answered && i === q.correct ? "bg-green-500 text-white" : answered && i === selected && selected !== q.correct ? "bg-red-500 text-white" : "bg-gray-200 dark:bg-white/10 text-gray-500"}`}
                  >
                    {" "}
                    {answered && i === q.correct ? (
                      <CheckCircle2 size={14} />
                    ) : answered && i === selected && selected !== q.correct ? (
                      <XCircle size={14} />
                    ) : (
                      OPTION_LABELS[i]
                    )}{" "}
          ...
          <AnimatePresence>
            ...
            {answered && (
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className={`p-4 rounded-2xl border ${isCorrect ? "bg-green-50 dark:bg-green-500/10 border-green-200 dark:border-green-500/20" : "bg-amber-50 dark:bg-[#F59E0B]/10 border-amber-200 dark:border-[#F59E0B]/20"}`}
              >
                {" "}
      ...
      <div className="flex flex-wrap gap-1.5 justify-center">
        ...
        {questions.map((_, i) => (
          <div
            key={i}
            className={`h-2 rounded-full transition-all ${i === current ? "w-4 bg-[#F59E0B]" : answers[i] !== null ? (answers[i] === questions[i].correct ? "w-2 bg-green-400" : "w-2 bg-red-400") : "w-2 bg-gray-200 dark:bg-white/20"}`}
          />
        ))}{" "}
      </div>{" "}
    </motion.div>
...
export default function GKWeeklyTest({
  onComplete,
  onBack,
}: GKWeeklyTestProps) {
  const [selectedWeek, setSelectedWeek] = React.useState<number | null>(null);
  const [questions, setQuestions] = React.useState<WeeklyQuestion[]>([]);
  ...
  const loadWeek = async (weekNum: number) => {
    ...
    try {
      ...
      setQuestions(data as WeeklyQuestion[]);
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 font-medium max-w-xs">
            {" "}
            Weekly quiz questions are being prepared by our GK team and will be
            available shortly. Check back later!{" "}
          </p>{" "}
        </div>{" "}
        <button
          onClick={() => {
          ...
          className="px-6 py-3 bg-[#F59E0B] text-[#060818] rounded-2xl font-black text-sm hover:shadow-lg hover:shadow-[#F59E0B]/20 transition-all"
        >
          {" "}
          ← Choose Another Week{" "}
        </button>{" "}
  ...
  if (!selectedWeek) {
    return <WeekSelector onSelect={loadWeek} onBack={onBack} />;
  }
  return (
    <QuizScreen
      questions={questions}
