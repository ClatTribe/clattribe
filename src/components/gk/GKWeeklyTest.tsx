...
// Program started on Monday, Jan 5 2026 (Week 1 = Jan 5–11).
// Every Monday a new week becomes available automatically — no manual updates needed.
const PROGRAM_START = new Date(2026, 0, 5); // Jan 5, 2026
...
interface WeekLabel {
  week: number;
  label: string;
  dateRange: string; // e.g. "Jan 5 – Jan 11"
}

function generateWeekLabels(): WeekLabel[] {
  const today = new Date();
  today.setHours(23, 59, 59, 999); // include today fully
  const weeks: WeekLabel[] = [];
  let weekStart = new Date(PROGRAM_START);
  let weekNum = 1;
...
function WeekSelector({ onSelect, onBack }: { onSelect: (week: number) => void; onBack?: () => void }) {
  const WEEK_LABELS = generateWeekLabels();
  return (
    <motion.div
      ...
      <div className="flex items-center justify-between">
        ...
        <div className={onBack ? '' : 'w-full'}>
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium text-sm mt-1">
            Fresh current affairs questions — new quiz every Monday.
          </p>
        </div>
      </div>
