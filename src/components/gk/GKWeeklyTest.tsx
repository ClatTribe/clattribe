...
const WEEK_LABELS = [
  { week: 1, label: 'Week 1', desc: 'Jan Wk 1' },
  { week: 2, label: 'Week 2', desc: 'Jan Wk 2' },
  { week: 3, label: 'Week 3', desc: 'Jan Wk 3' },
  { week: 4, label: 'Week 4', desc: 'Jan Wk 4' },
  { week: 5, label: 'Week 5', desc: 'Feb Wk 1' },
  { week: 6, label: 'Week 6', desc: 'Feb Wk 2' },
  { week: 7, label: 'Week 7', desc: 'Feb Wk 3' },
  { week: 8, label: 'Week 8', desc: 'Feb Wk 4' },
  { week: 9, label: 'Week 9', desc: 'Mar Wk 1' },
  { week: 10, label: 'Week 10', desc: 'Mar Wk 2' },
  { week: 11, label: 'Week 11', desc: 'Mar Wk 3' },
  { week: 12, label: 'Week 12', desc: 'Mar Wk 4' },
];

function WeekSelector({ onSelect, onBack }: { onSelect: (week: number) => void; onBack?: () => void }) {
  return (
    <motion.div
        ...
        {WEEK_LABELS.map((w, idx) => (
          ...
          >
            <div className="w-12 h-12 bg-[#F59E0B]/10 rounded-2xl flex items-center justify-center group-hover:bg-[#F59E0B] transition-colors">
              <Calendar size={22} className="text-[#F59E0B] group-hover:text-[#060818] transition-colors" />
            </div>
            <div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-gray-50 dark:border-white/5">
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">20 Qs</span>
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className={`flex items-center gap-2 px-4 py-2 rounded-2xl font-black text-sm ${
          timeLeft < 60 ? 'bg-red-500/10 text-red-500' : 'bg-[#F59E0B]/10 text-[#F59E0B]'
        }`}>
          <Timer size={16} />
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="space-y-3">
            {q.options.map((opt, i) => {
              let style = 'bg-gray-50 dark:bg-white/5 border-gray-100 dark:border-white/10 text-gray-700 dark:text-gray-300 hover:border-[#F59E0B]/40';
              if (answered) {
...
export default function GKWeeklyTest({ onComplete, onBack }: GKWeeklyTestProps) {
  ...
  const loadWeek = async (weekNum: number) => {
    ...
    try {
      ...
      if (err) throw err;
      if (!data || data.length === 0) throw new Error('No questions found for this week.');
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
        ...
        <button
          onClick={() => { setError(null); setSelectedWeek(null); }}
