...
import GKSubscriptionModal, { MockPaywallModal } from './SubscriptionModal';
...
export default function GKApp() {
  ...
  React.useEffect(() => {
    ...
    setHasMockAccess(localStorage.getItem('gk_hasMockAccess') === 'true');
  }, []);
  ...
  const handleUnlockMocks = () => {
    localStorage.setItem('gk_hasMockAccess', 'true');
    setHasMockAccess(true);
    setShowMockModal(false);
  };

  if (!isLoggedIn) return <GKLogin onLogin={handleLogin} />;

  const renderContent = () => {
    switch (activeTab) {
      case 'dashboard': return <GKDashboard />;
      case 'testing':
        return <GKTestingEngine hasMockAccess={hasMockAccess} onUnlockMocks={() => setShowMockModal(true)} />;
        ...
        return (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-8 font-bold uppercase tracking-widest text-xs">
              CLAT 2025 Aspirant •{' '}
              {hasMockAccess ? 'Pro + Mocks' : isSubscribed ? 'Pro Member' : 'Free Trial'}
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-12">
              {!isSubscribed && (
                <button onClick={() => setShowSubModal(true)} className="bg-[#F59E0B] text-[#060818] px-10 py-4 rounded-2xl font-black shadow-xl hover:scale-[1.02] transition-all">
                  Upgrade to Pro — ₹99/mo
                </button>
              )}
              {isSubscribed && !hasMockAccess && (
                <button onClick={() => setShowMockModal(true)} className="bg-purple-600 text-white px-10 py-4 rounded-2xl font-black shadow-xl hover:scale-[1.02] transition-all">
  ...
  return (
    <>
      ...
      {showSubModal && <GKSubscriptionModal onSubscribe={handleSubscribe} onClose={() => setShowSubModal(false)} />}
      {showMockModal && <MockPaywallModal onUnlock={handleUnlockMocks} onClose={() => setShowMockModal(false)} />}
    </>
  );
}
