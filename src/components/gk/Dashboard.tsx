...
export default function GKDashboard() {
  ...
  React.useEffect(() => {
    const stored = localStorage.getItem('gk_userName');
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="grid gap-4">
            {GK_DAILY_TARGETS.map((target, idx) => (
              <motion.div
                key={target.id}
    ...
    <div className="space-y-12 relative">
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-4 bg-gray-50 dark:bg-white/5 p-2 rounded-2xl border border-gray-100 dark:border-white/5">
          <div className="flex -space-x-2">
            {[1, 2, 3].map((i) => (
              <div key={i} className="w8 h-8 rounded-full border-2 border-white dark:border-[#060818] bg-gray-200 dark:bg-gray-800" />
            ))}
          </div>
          <span className="text-xs font-bold text-gray-500 dark:text-gray-400 pr-2">12+ friends studying now</span>
        </div>
              ...
              <motion.div
                ...
                className={`flex items-center gap-4 p-3 rounded-2xl transition-all ${
                  student.is_user
                    ...
                    : 'hover:bg-gray-50 dark:hover:bs-white/5'
                }`}
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-xs ${
                  student.rank === 1 ? 'bg-amber-400 text-[#060818] shadow-lg shadow-amber-400/20' :
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <section className="bg-[#060818] text-white p-10 rounded-[3rem] relative overflow-hidden group">
          ...
          <div className="absolute top-0 right-0 w-64 h-64 bg-[#F59E0B]/10 rounded-full -mr-32 -mt-32 blur-3xl group-hover:bw-[#F59E0B]/20 transition-all duration-700" />
        </section>
