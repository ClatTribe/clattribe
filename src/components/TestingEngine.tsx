...
import NLATMockTest from './NLATMockTest';
import NLATScorecard from './NLATScorecard';
import { WeeklyQuizListing, WeeklyQuizRunner } from './WeeklyQuizEngine';
import { nlatMocks } from '../data/nlat';
import { NLATMock, NLATTestResult } from '../data/nlat/types';
...
const CATEGORIES: TestCategory[] = [
  ...
  },
  {
    ...
    description: '120-minute NLAT & CLAT simulated exams. Section-wise scores, answer review, and no negative marking for NLAT.',
    icon: <Target size={24} />,
    count: '15 NLAT Mocks',
    color: 'bg-purple-500',
    accent: 'text-white'
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <h2 className="text-2xl font-black text-brand-navy dark:text-white">Configure Custom Test</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-8">
          ...
          {/* Start Button */}
          <div className="bg-brand-navy p-8 rounded-[2.5rem] space-y-6">
            ...
            <button
              disabled={isBuilding}
              onClick={handleStart}
              className="w-full bg-brand-amber text-brand-navy py-4 rounded-2xl font-black text-lg shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-50"
            >
              {isBuilding ? (
                <motion.div 
                  animate={{ rotate: 360 }}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <h2 className="text-2xl font-black text-brand-navy dark:text-white">Performance Analysis</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-brand-navy p-10 rounded-[3rem] text-center space-y-6 shadow-2xl">
          ...
          <div className="pt-4 border-t border-white/10 flex justify-between items-center">
            ...
            <p className="text-brand-amber font-black text-sm uppercase tracking-widest">
        ...
        <div className="lg:col-span-2 space-y-8">
              ...
              {result.topicBreakdown.map((item, i) => (
                <div key={i} className="space-y-2">
                  <div className="flex justify-between text-sm font-bold">
                    <span className="text-brand-navy dark:text-white">{item.topic}</span>
                    <span className="text-gray-500">{item.correct}/{item.total}</span>
                  </div>
                  <div className="h-2 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden">
                    <motion.div 
                      className={`h-full ${item.correct === item.total ? 'bg-green-500' : 'bg-brand-amber'}`}
                      initial={{ width: 0 }}
          ...
          {/* Suggestions */}
          <div className="bg-amber-50 dark:bg-brand-amber/10 p-8 rounded-[2.5rem] border border-amber-100 dark:border-brand-amber/20 space-y-4">
            <h3 className="text-lg font-black text-brand-amber flex items-center gap-2">
              <Zap size={20} fill="currentColor" />
              Areas for Improvement
            </h3>
            <ul className="space-y-3">
              {result.suggestions.map((suggestion, i) => (
                <li key={i} className="flex items-start gap-3 text-sm font-medium text-amber-900/70 dark:text-brand-amber/70">
      ...
      <div className="flex justify-center gap-4">
        <button 
          onClick={onBack}
          className="px-10 py-4 bg-white dark:bg-white/10 border border-gray-100 dark:border-white/10 text-brand-navy dark:text-white rounded-2xl font-bold hover:bg-gray-50 dark:hover:bg-white/20 transition-all"
        >
          Retake Test
        </button>
        <button 
          onClick={onBack}
          className="px-10 py-4 bg-brand-amber text-brand-navy rounded-2xl font-bold hover:shadow-xl hover:shadow-brand-amber/20 transition-all"
        >
          Explore More Tests
        </button>
      </div>
...
export default function TestingEngine() {
  ...
  // NLAT state
  const [activeMock, setActiveMock] = React.useState<NLATMock | null>(null);
  const [nlatResult, setNlatResult] = React.useState<NLATTestResult | null>(null);
  ...
  // ââ NLAT Scorecard ââââââââââââââââââââââââââââââââââââââ
  if (nlatResult) {
    return (
      <NLATScorecard
        result={nlatResult}
        onBack={() => { setNlatResult(null); setActiveMock(null); setActiveCategory(null); }}
        onRetake={() => { setNlatResult(null); setActiveMock(null); }}
      />
    );
  }

  // ââ NLAT Active Test âââââââââââââââââââââââââââââââââââââ
  if (activeMock) {
    return (
      <NLATMockTest
        mock={activeMock}
  ...
  // ââ Existing passage test ââââââââââââââââââââââââââââââââ
  if (testResult) {
  ...
  if (activeCategory === 'passage') {
    return (
      <div className="space-y-6">
        ...
        <PassageTest onComplete={handleTestComplete} />
      </div>
    );
  }

  // ââ Full Mock Tests listing ââââââââââââââââââââââââââââââ
  if (activeCategory === 'mock') {
    return (
      <FullMockListing
        nlatMocks={nlatMocks}
        onStartNLAT={(mock) => setActiveMock(mock)}
        onBack={() => setActiveCategory(null)}
      />
    );
  }

  // ââ Weekly Quizzes âââââââââââââââââââââââââââââââââââââââ
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white dark:bg-white/5 p-12 rounded-[3rem] border border-gray-100 dark:border-white/10 text-center space-y-6">
          <div className="w-20 h-20 bg-brand-amber/10 rounded-full flex items-center justify-center mx-auto">
            <Timer size={40} className="text-brand-amber" />
          </div>
          <h2 className="text-3xl font-black text-brand-navy dark:text-white">
  ...
  return (
    <div className="space-y-12">
      {/* Header */}
      <div className="space-y-2">
        <h1 className="text-4xl font-black text-brand-navy dark:text-white tracking-tight">
          Testing Engine
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-brand-navy dark:bg-white/5 p-6 rounded-3xl border border-white/10 flex items-center gap-4">
          <div className="w-12 h-12 bg-brand-amber rounded-2xl flex items-center justify-center text-brand-navy">
            <Trophy size={24} />
          </div>
          <div>
      ...
      {insights && (insights.weakAreas.length > 0 || insights.slowAreas.length > 0) && (
        <section className="space-y-6">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-brand-amber/10 rounded-lg flex items-center justify-center text-brand-amber">
              <Lightbulb size={18} />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  {insights.weakAreas.slice(0, 2).map(area => (
                    <div key={area.topic} className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-black text-brand-navy dark:text-white">{area.topic}</p>
            ...
            {insights.slowAreas.length > 0 && (
              <div className="bg-amber-50 dark:bg-brand-amber/5 p-8 rounded-[2.5rem] border border-amber-100 dark:border-brand-amber/10 space-y-4">
                <div className="flex items-center gap-2 text-brand-amber font-black text-[10px] uppercase tracking-widest">
                  <Clock size={14} />
                  Speed Bottlenecks
                </div>
                <div className="space-y-4">
                  {insights.slowAreas.slice(0, 2).map(area => (
                    <div key={area.topic} className="flex items-center justify-between">
                      ...
                      <span className="text-lg font-black text-brand-amber">Slow</span>
                    </div>
                  ))}
                </div>
                <button 
                  onClick={() => setActiveCategory('passage')}
                  className="w-full py-3 bg-brand-amber text-brand-navy rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-amber-400 transition-all"
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
          {CATEGORIES.map((cat) => (
            <motion.button
              ...
              className="group relative bg-white dark:bg-white/5 p-8 rounded-[2.5rem] border border-gray-100 dark:border-white/10 text-left hover:shadow-2xl hover:shadow-brand-amber/10 transition-all duration-300 overflow-hidden"
            >
              {/* Background Accent */}
              ...
              <div className="space-y-6 relative">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <h3 className="text-xl font-black text-brand-navy dark:text-white group-hover:text-brand-amber transition-colors">
                      {cat.title}
                    </h3>
                    <ChevronRight size={20} className="text-gray-300 group-hover:text-brand-amber transition-all group-hover:translate-x-1" />
                  </div>
          ...
          <motion.div 
            whileHover={{ y: -8 }}
            className="bg-brand-amber p-8 rounded-[2.5rem] flex flex-col justify-between relative overflow-hidden group cursor-pointer"
          >
            <div className="absolute top-0 right-0 p-4 opacity-20 group-hover:scale-125 transition-transform duration-700">
              <Zap size={120} className="text-brand-navy" />
            </div>
            <div className="space-y-4 relative">
              <h3 className="text-2xl font-black text-brand-navy leading-tight">Create Custom Test</h3>
              <p className="text-brand-navy/60 text-sm font-bold">Mix topics and set your own timer for a personalized challenge.</p>
            </div>
            <button 
              onClick={() => setActiveCategory('custom')}
              className="bg-brand-navy text-white px-6 py-3 rounded-2xl font-black text-sm w-fit mt-8 relative group-hover:bg-gray-800 transition-colors"
            >
              Build Test
            </button>
          </motion.div>
        ...
        <div className="space-y-8">
          <div className="flex items-center justify-between">
            ...
            <History size={20} className="text-brand-amber" />
          </div>
            ...
            {history.length === 0 ? (
              ...
              history.map((test, idx) => (
                <motion.button
                  ...
                  className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-gray-50 dark:hover:bg-white/5 transition-all text-left group border border-transparent hover:border-brand-amber/20"
                >
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-black text-brand-navy dark:text-white group-hover:text-brand-amber transition-colors">
                      {test.category}
                    </p>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{test.date}</p>
                  </div>
                  <ChevronRight size={16} className="text-gray-300 group-hover:text-brand-amber transition-all group-hover:translate-x-1" />
                </motion.button>
              ))
            )}
          </div>
